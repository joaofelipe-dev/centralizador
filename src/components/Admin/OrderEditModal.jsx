...
export function OrderEditModal({ order, isOpen, onClose, onSave }) {
  ...
  const updateItem = (productId, field, delta) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError(err.message || 'Erro ao salvar ajustes');
    } finally {
      setIsSaving(false);
    }
  };
        ...
        <div className="p-6 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">{order.store?.name} • Pedido #{order.id.split('-')[0]}</p>
            </div>
          </div>
